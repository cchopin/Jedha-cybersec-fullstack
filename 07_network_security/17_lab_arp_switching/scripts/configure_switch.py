#!/usr/bin/env python3
"""
Script pour configurer un switch Cisco IOU pour le lab ARP/Switching
...
def configure_switch(host, port, hostname, vlans, trunk_interface, trunk_vlans,
                    native_vlan, vlan_ips=None, netmask="255.255.255.0"):
    """Configure le switch avec les VLANs, trunk et interfaces VLAN pour ARP"""
    ...
    # Configurer les interfaces VLAN avec des IPs pour les tests ARP
    if vlan_ips:
        print("[*] Configuration des interfaces VLAN (pour tests ARP)...")
        for vlan_name, ip in vlan_ips.items():
            vlan_id = vlan_name.replace("vlan", "")
            print(f"    - Interface VLAN {vlan_id}: {ip}")
            tn.send_command(f"interface vlan {vlan_id}")
            tn.send_command(f"ip address {ip} {netmask}")
            tn.send_command("no shutdown")
            tn.send_command("exit")
    ...
    output = tn.send_command("show ip interface brief", wait=1)
    print(output)
...
def main():
    parser = argparse.ArgumentParser(description='Configure Cisco IOU Switch for ARP Lab')
    ...
    parser.add_argument('--vlan-ips', help='VLAN IPs JSON object (optional)')
    parser.add_argument('--netmask', default='255.255.255.0', help='Netmask for VLAN IPs')

    args = parser.parse_args()

    vlans = json.loads(args.vlans)
    vlan_ips = json.loads(args.vlan_ips) if args.vlan_ips else None
